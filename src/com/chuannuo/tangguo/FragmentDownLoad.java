...
public class FragmentDownLoad extends BaseFragment {
	...
	private LinearLayout linearLayout9; //upload image
	...
	private void initView(){
		...
		iv_logo.setImageBitmap(ResourceUtil.getImageFromAssetsFile(getActivity(), "tangguo.png"));
		...
		LinearLayout.LayoutParams lpUpload = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
		...
		rl_upload.setBackgroundColor(Color.parseColor(Constant.ColorValues.UPLOAD_IMG_BCAK));
		...
		tv_example.setTextColor(Color.parseColor(Constant.ColorValues.LIGHT_RED));
		tv_example.setTextSize(20);
		tv_example.setTypeface(Typeface.DEFAULT_BOLD);
		
		iv_upload.setBackgroundColor(Color.parseColor(Constant.ColorValues.UPLOAD_IMG_BCAK));
		iv_upload.setImageBitmap(ResourceUtil.getImageFromAssetsFile(getActivity(), "upload.png"));
...
//				
				if (appInfo.getClicktype() == 1) {
					...
					tv_tips3.setText("按下面示例图截图上传即可获得 "+ appInfo.getPhoto_integral()+appInfo.getTextName() +"，（注意只有一次上传机会，请严格按照要求上传）");
					...
					if (appInfo.getScore() > 0) {
						str = appInfo.getAlert() + appInfo.getScore()+appInfo.getTextName();
					}
					if (appInfo.getIs_photo() == 1 || appInfo.isSign()) {
						tv_tips1.setText(str+"请到未完成任务列表中,按下面示例图截图上传即可获得 "+ appInfo.getPhoto_integral()+appInfo.getTextName() + "，（注意只有一次上传机会，请严格按照要求上传）");
						rl_upload.setVisibility(View.VISIBLE);
						if(appInfo.isSign()){
							iv_upload.setVisibility(View.VISIBLE);
						}else{
...
//							+ "天可以签到 一次，签到" + appInfo.getNeedSign_times() + "次，任务完成");
	...
	private void loadImage() {
		...
		Drawable cachedImage1 = SyncImageLoader.getInstance().loadDrawable(appInfo.getH5_big_url(), new ImageCallback() {  
            public void imageLoaded(Drawable imageDrawable, String imageUrl) {
                	iv_example.setImageDrawable(imageDrawable);
            }  
        });
        if (cachedImage1 == null) {  
        	iv_example.setImageBitmap(ResourceUtil.getImageFromAssetsFile(getActivity(), "tangguo.png"));
        }else{  
